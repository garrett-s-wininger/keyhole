...
constexpr auto jdk_version(const kh::jvm::classfile::Version version) noexcept
    -> uint8_t {
  if (version.major < 49) {
...
auto attachment_targets() -> kh::argparse::CommandResult {
  ...
  const auto directory_iterator = std::filesystem::directory_iterator{
      tempdir, std::filesystem::directory_options::follow_directory_symlink |
  ...
  const auto jvm_perf_dir =
      std::filesystem::path{std::format("hsperfdata_{}", username)};
    ...
    if (entry.path().filename() == jvm_perf_dir) {
      if (!entry.is_directory()) {
        return kh::argparse::fatal(
            std::format("User performance data location ({}) not a directory",
                        entry.path().string()));
      }

      found = true;

      const auto subdir_iterator =
...
auto inspect_class_file(std::string_view target)
    -> kh::argparse::CommandResult {
  auto result = kh::jvm::parsing::load_class_from_file(target);

  if (!result) {
    return kh::argparse::fatal(
        std::format("Failed to parse class from file ({})", target));
  }
  ...
  std::println("  Name         - {} ({})", klass.name(), klass.superclass());

  std::println("  Version      - {}.{} (Java {})", klass.version.major,
               klass.version.minor, jdk_version(klass.version));

  std::println("  Access Flags - 0x{:04X}", klass.access_flags);
  ...
  if (klass.attributes.size() > 0) {
    ...
    for (const auto &attribute : klass.attributes) {
      std::println("  {}", klass.constant_pool
                               .resolve<kh::jvm::constant_pool::UTF8Entry>(
...
auto write_modified_class(std::string_view target)
    -> kh::argparse::CommandResult {
  ...
  const auto instructions =
      kh::jvm::isa::InstructionSequence{kh::jvm::isa::Return};

  const auto mutation_result =
      klass.add_method("generatedAction"sv, "()V"sv, instructions);
  ...
  const auto prefix_result = klass.prefix_method(
      "empty"sv, "()V"sv,
      {kh::jvm::isa::Nop, kh::jvm::isa::Nop, kh::jvm::isa::Nop});
  ...
  const auto replacement_result =
      klass.replace_method("main"sv, "([Ljava/lang/String;)V"sv, instructions);
  ...
  if (!stream) {
    return kh::argparse::fatal(std::format("Failed to open requested file ({})",
...
auto main(const int argc, const char **argv) -> int {
  using AttachmentTargetsCommand =
      kh::argparse::Command<"attachment-targets", ::attachment_targets>;

  using InspectCommand = kh::argparse::Command<"inspect", ::inspect_class_file>;
  using ModifyCommand =
      kh::argparse::Command<"modify-class", ::write_modified_class>;

  try {
    const auto result =
        kh::argparse::CLI<AttachmentTargetsCommand, InspectCommand,
                          ModifyCommand>{
            .name = "KeyHole CLI",
            .version = "0.1.0",
            .description =
                "Provides instrumentation and introspection for JVM bytecode",
        }
            .execute(argc, argv);
    ...
    std::println(stderr, "[ERROR] An unhandled exception occurred:\n\n  {}",
                 e.what());
