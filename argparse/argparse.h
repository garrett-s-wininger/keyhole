...
auto fatal(std::string &&message) noexcept -> CommandResult {
  return std::unexpected(
      Error{Error::Type::CommandExecutionFailure, std::move(message)});
}
...
template <std::size_t S> struct NonTypeTemplateString {
  char buffer[S];
  ...
  constexpr operator std::string_view() const { return {buffer, S - 1}; }
};
...
struct Command {
  ...
  static auto execute(std::string_view path, std::span<const char *> args)
    ...
    return std::unexpected(
        Error{Error::Type::MalformedArgumentList, std::string{error_message}});
  }
...
template <typename... Subcommands> struct CLI {
  static_assert(sizeof...(Subcommands) > 0,
  ...
  auto execute(const int argc, const char **argv)
    ...
    const auto args =
        std::span<const char *>(argv, static_cast<std::size_t>(argc));
    ...
    if (args.size() < 2) {
      ...
      return std::unexpected(Error{Error::Type::MalformedArgumentList,
    ...
    (
        [args, &head, &next, &dispatch_result] {
          if (Subcommands::name == next) {
            dispatch_result = Subcommands::execute(head, args.subspan(1));
          } else {
            return;
          }
        }(),
    ...
    return dispatch_result.value_or(std::unexpected(Error{
        Error::Type::InvalidSubcommand,
        std::format("The requested subcommand ({}) is not registered", next)}));
  }
  ...
  auto usage(std::string_view program_name, std::FILE *destination) -> void {
    ...
    ((std::println(destination, "  {} {} [<ARGS>]", program_name,
                   Subcommands::name)),
     ...);
  }
};
