...
auto parse_attribute(reader::Reader &reader) noexcept
  ...
  const auto header =
      reader.read_bytes(sizeof(std::uint32_t) + sizeof(std::uint16_t));
  ...
  return kh::jvm::attribute::Attribute{name_index, body.value()};
}
...
auto parse_method(kh::reader::Reader &reader)
  ...
  return kh::jvm::method::Method{access_flags, name_index, descriptor_index,
                                 std::move(attributes)};
}
...
auto parse_code_attribute(kh::reader::Reader &reader) noexcept
  ...
  const auto exception_table_result =
      reader.read_bytes(exception_table_size_result.value());
  ...
  return kh::jvm::attribute::CodeAttribute{.max_operand_stack_size = max_stack,
                                           .max_local_variables = max_locals,
                                           .bytecode = bytecode_result.value(),
                                           .exception_table =
                                               exception_table_result.value(),
...
auto parse_class_file(kh::reader::Reader &reader)
  ...
  const auto header =
      reader.read_bytes(sizeof(std::uint64_t) + sizeof(std::uint16_t));
  ...
  const auto access_and_class_metadata =
      reader.read_bytes(sizeof(std::uint64_t));
