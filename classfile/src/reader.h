...
public:
  ...
  auto read_bytes(uint32_t) -> std::expected<std::span<const std::byte>, Error>;

  template <kh::endian::MultiByteIntegral V> auto read_unchecked() -> V {
    ...
    return kh::endian::big(std::bit_cast<V>(buffer));
  }
