...
auto serialize(kh::sinks::Sink auto &sink,
...
auto serialize(kh::sinks::Sink auto &sink,
               const kh::jvm::constant_pool::MethodReferenceEntry entry)
    -> void {
  sink.write(static_cast<std::uint8_t>(constant_pool::tag(entry)));
...
auto serialize(kh::sinks::Sink auto &sink,
  ...
  for (const auto &entry : pool.entries()) {
    std::visit([&sink](const auto &e) { serialize(sink, e); }, entry);
  }
}
...
auto serialize(kh::sinks::Sink auto &sink,
  ...
  sink.write(
      static_cast<std::uint16_t>(klass.constant_pool.entries().size() + 1));
