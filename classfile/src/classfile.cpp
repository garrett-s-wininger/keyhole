...
ClassFile::ClassFile() noexcept
    : version{Version{55u, 0u}}, class_index{0u}, superclass_index{0u},
      constant_pool(kh::jvm::constant_pool::ConstantPool{}),
      access_flags(0x0021), methods(std::vector<kh::jvm::method::Method>{}),
      attributes(std::vector<kh::jvm::attribute::Attribute>{}) {}

auto ClassFile::add_method(std::string_view name, std::string_view descriptor,
  ...
  auto code_attribute =
      kh::jvm::attribute::Attribute(constant_pool.try_add_utf8_entry("Code"),
                                    kh::jvm::isa::generate_code(code));

  methods.push_back(kh::jvm::method::Method{
      .access_flags = static_cast<std::uint16_t>(AccessFlags::ACC_PUBLIC |
                                                 AccessFlags::ACC_STATIC |
                                                 AccessFlags::ACC_SYNTHETIC),
      .name_index =
          static_cast<std::uint16_t>(constant_pool.try_add_utf8_entry(name)),
...
auto ClassFile::method(std::string_view name)
    -> std::optional<std::reference_wrapper<kh::jvm::method::Method>> {
  return method(name, "()V");
}

auto ClassFile::method(std::string_view name, std::string_view descriptor)
    -> std::optional<std::reference_wrapper<kh::jvm::method::Method>> {
  auto candidates =
      methods | std::views::filter([this, name, descriptor](auto &method) {
...
auto ClassFile::name() -> std::string_view {
  const auto &class_entry =
      constant_pool.resolve<constant_pool::ClassEntry>(class_index);

  return constant_pool
      .resolve<kh::jvm::constant_pool::UTF8Entry>(class_entry.name_index)
      .text;
}

auto ClassFile::prefix_method(
  ...
  auto attribute_search_result =
      kh::jvm::views::MethodView{constant_pool, m}.attribute("Code");
  ...
  // have operands
  merged_instructions.reserve(instructions.size() +
                              existing_code.bytecode.size());

  merged_instructions.insert(merged_instructions.end(), instructions.begin(),
                             instructions.end());

  auto reader = kh::reader::Reader{existing_code.bytecode};

  if (auto parse_result = parsing::parse_bytecode(reader); parse_result) {
    merged_instructions.insert(merged_instructions.end(),
...
auto ClassFile::rename(std::string_view new_name) -> void {
  const auto name_index = constant_pool.try_add_utf8_entry(new_name);
  auto &class_constant =
      constant_pool.resolve<kh::jvm::constant_pool::ClassEntry>(class_index);
...
auto ClassFile::replace_method(
  ...
  const auto attribute_search_result =
      kh::jvm::views::MethodView{constant_pool, method}.attribute("Code");
  ...
  auto new_code_attribute = kh::jvm::attribute::Attribute(
      ...
      kh::jvm::isa::generate_code(existing_code.max_operand_stack_size,
                                  existing_code.max_local_variables, new_code));
...
auto ClassFile::superclass() -> std::string_view {
  const auto &class_entry =
      constant_pool.resolve<kh::jvm::constant_pool::ClassEntry>(
          superclass_index);
