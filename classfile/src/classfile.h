...
struct ClassFile {
  ...
  ClassFile(S1 &&class_name, S2 &&superclass_name) : ClassFile() {
    constant_pool.add(kh::jvm::constant_pool::UTF8Entry{class_name});

    constant_pool.add(kh::jvm::constant_pool::ClassEntry{1});
    class_index = 2;

    constant_pool.add(kh::jvm::constant_pool::UTF8Entry{superclass_name});

    constant_pool.add(kh::jvm::constant_pool::ClassEntry{3});
    superclass_index = 4;
  }

  auto add_method(std::string_view, std::string_view,
  ...
  auto prefix_method(std::string_view, std::string_view,
                     const kh::jvm::isa::InstructionSequence &)
      -> std::expected<void, Error>;

  auto rename(std::string_view) -> void;

  auto replace_method(std::string_view, std::string_view,
                      const kh::jvm::isa::InstructionSequence &)
      -> std::expected<void, Error>;
