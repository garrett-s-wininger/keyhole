...
TEST(Serialization, SerializesAttribute) {
  constexpr std::array<const std::byte, 3> data{std::byte{'A'}, std::byte{'B'},
                                                std::byte{'C'}};

  const attribute::Attribute attribute{12u, data};
...
TEST(Serialization, SerializesCodeAttribute) {
  ...
  constexpr auto expected = std::to_array({// Max stack values
...
TEST(Serialization, SerializesMethod) {
  const method::Method method{
      static_cast<uint16_t>(method::AccessFlags::ACC_PUBLIC), 3u, 4u,
      std::vector<attribute::Attribute>{
          attribute::Attribute(5u, std::span<const std::byte>{})}};

  kh::sinks::VectorSink sink{};
  serialize(sink, method);

  constexpr auto expected =
      std::to_array({// Method access
                     std::byte{0x00}, std::byte{0x01},
                     // Name index
...
TEST(Serialization, SerializesConstantPool) {
  ...
  const constant_pool::ConstantPool pool{constant_pool::ClassEntry{2u},
  ...
  constexpr auto expected = std::array<const std::byte, 7>{
      // Class entry
      std::byte{0x07}, std::byte{0x00}, std::byte{0x02},
      // UTF8 entry
...
TEST(Serialization, SerializesMethodReferenceEntries) {
  ...
  constexpr auto expected =
      std::array<const std::byte, 5>{// Tag
                                     std::byte{0x0A},
                                     // Class index
                                     std::byte{0x00}, std::byte{0x01},
                                     // Descriptor index
...
TEST(Serialization, SerializesUTF8Entries) {
  ...
  constexpr auto expected = std::array<const std::byte, 10>{
      std::byte{0x01}, std::byte{0x00}, std::byte{0x07}, std::byte{'M'},
      std::byte{'y'},  std::byte{'C'},  std::byte{'l'},  std::byte{'a'},
...
TEST(Serialization, SerializesClassFile) {
  ...
  klass.constant_pool.add(kh::jvm::constant_pool::UTF8Entry{method_name});

  klass.constant_pool.add(kh::jvm::constant_pool::UTF8Entry{descriptor});

  const auto attribute_name = std::string{"Deprecated"};

  klass.constant_pool.add(kh::jvm::constant_pool::UTF8Entry{attribute_name});

  auto deprecated_attribute =
      kh::jvm::attribute::Attribute(7, std::span<const std::byte>{});

  klass.methods.push_back(kh::jvm::method::Method{
      .access_flags =
      ...
      .descriptor_index = 6,
      .attributes =
          std::vector<kh::jvm::attribute::Attribute>{deprecated_attribute}});
