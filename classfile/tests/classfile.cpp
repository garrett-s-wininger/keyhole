...
TEST(ClassFile, ReplaceMethodReplacesExistingCode) {
  ...
  const auto original_code =
      kh::jvm::isa::InstructionSequence{kh::jvm::isa::Return};

  std::ignore = klass.add_method(name, descriptor, original_code);
  const auto new_code = kh::jvm::isa::InstructionSequence{kh::jvm::isa::Nop,
                                                          kh::jvm::isa::Return};

  const auto replace_result = klass.replace_method(name, descriptor, new_code);
  ...
  const auto code_search_result =
      kh::jvm::views::MethodView{klass.constant_pool, method}.attribute("Code");
...
TEST(ClassFile, ReplaceMethodFailsIfNonExistent) {
  ...
  const auto new_code = kh::jvm::isa::InstructionSequence{kh::jvm::isa::Return};
...
TEST(ClassFile, PrefixMethodPrependsInstructions) {
  ...
  const auto code = kh::jvm::isa::InstructionSequence{kh::jvm::isa::Return};

  std::ignore = klass.add_method(name, descriptor, code);

  const auto prefix_code = kh::jvm::isa::InstructionSequence{kh::jvm::isa::Nop};
