...
auto
Reader::read_byte() -> std::expected<const std::byte, Error> {
    if (remaining_.empty()) {
        return std::unexpected{Error::EndOfStream};
    }

    const auto byte = remaining_.front();
...
auto
Reader::read_bytes(std::uint32_t count)
